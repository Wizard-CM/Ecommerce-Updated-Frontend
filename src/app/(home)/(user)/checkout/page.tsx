...
const stripePromise = loadStripe(
  process.env.NEXT_PUBLIC_STRIPE_PROMISE || ""
);
        ...
        <div className="bg-gray-800 backdrop-blur-sm bg-opacity-90 rounded-2xl shadow-2xl p-8 border border-gray-700 transition-all duration-300 hover:shadow-indigo-500/10">
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-8 flex items-center">
            <span className="relative flex mr-4">
              ...
              <svg className="w-8 h-8 text-indigo-400 relative" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <form onSubmit={submitHandler} className="space-y-6">
            ...
            <button 
              type="submit" 
              disabled={processing}
              className={`w-full py-4 px-6 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-medium text-lg transition-all duration-300 transform hover:scale-[1.02] hover:shadow-lg hover:shadow-purple-500/20 disabled:opacity-70 disabled:cursor-not-allowed ${processing ? 'animate-pulse' : ''}`}
            >
              {processing ? (
...
const OrderSummary = ({ products, shippingCharge, tax, subTotal, discount, total }: OrderSummaryProps) => (
        ...
        <div className="mb-6">
          <div className="space-y-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
            {products.map((product: productType) => (
              <div key={product._id} className="flex items-center text-gray-200 py-2 px-3 rounded-lg bg-gray-700 bg-opacity-50 border border-gray-600">
                {product.photo && (
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-indigo-300">${product.price?.toFixed(2)} × {product.quantity || 1}</p>
                </div>
